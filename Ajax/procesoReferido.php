...
if (isset($_SESSION['idusuario'])) {
    ...
    $numeroReferido          = str_replace("'", "", $data['numRef']);
    ...
    if ($devuelveRef == 0 and $devuelveAge == 0 and $devuelveEstaEnUso == 0 and $devuelveListaNegra == 0 and $devuelveEstaRefCuaderno == 0) {
        ...
        if (($devuelveSeUso > 0 && $flag == 'libre') or ($devuelveSeUso == 0)) {
            $nombre      = str_replace("'", "", $data['nomRef']);
            $observacion = str_replace("'", "", $data['obsRef']);
            $chk         = '';
            ...
            if ($result) {
                $qBorrarReferido      = "delete from referidos where numero = '$numeroReferido'";
                $resultBorrarReferido = mysqli_query($mysqli, $qBorrarReferido);
                $q2                   = "insert into historico values(null,'$numeroReferido','referido',$idusuario,'$fecha','$chk')";
                $result2              = mysqli_query($mysqli, $q2);
                $idhistorico          = mysqli_insert_id($mysqli);
                if ($result2) {
                    $q3      = "insert into referidos values(null,$idusuario,$idhistorico,'$numeroReferido','$nombre','$observacion','$fecha',0)";
