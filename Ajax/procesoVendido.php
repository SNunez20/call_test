...
if (isset($_SESSION['idusuario'])) {
    ...
    // Compruebo si el número proviene de una llamada entrante
    $numero = $data["llamadaEntrante"] == "true" ? $_POST["numero"] : $_SESSION["numero"];
    ...
    $q2          = "insert into historico values(null,'$numero','vendido',$idusuario,'$fecha','$chk')";
    ...
    $q      = "update numeros set flag = 'vendido' where numero = '$numero'";
    ...
    if ($result) {
        ...
        $iddetalle = mysqli_insert_id($mysqli);
        if ($result3) {
            $qVendido      = "insert into vendidos values(null,$idusuario,$idhistorico,$iddetalle,'$fecha')";
            $resultVendido = mysqli_query($mysqli, $qVendido);
            ...
            if ($resultGrupo) {
                if ($cantGrupos > 0) {
                    for ($i = 0; $i < count($grupos); $i++) {
                        $q5 = "select * from numeros where grupo = '$grupos[$i]' and flag = 'libre' order by rand() LIMIT 1";

                        // $q5 = "select n.numero, n.flag,n.dep_localidad, h.fecha from numeros as n
                        ...
                        $devuelve = mysqli_num_rows($result5);
                        if ($devuelve > 0) {
                            $i = count($grupos);
                        }
                    }
            ...
            if ($devuelve > 0) {
                ...
                if ($result5) {
                    ...
                    if ($result6) {
                        ...
                        if ($result7) {
                            $response = array('result' => 'NumeroDespuesVendido', 'numero' => $numero, 'message' => 'Correcto.', 'localidad' => $localidad);
                        }
                    }
                }
            } else {
