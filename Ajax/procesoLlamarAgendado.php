...
if (isset($_SESSION['idusuario'])) {
    ...
    $_SESSION["numero"] = $numeroAgendado;
    ...
    if ($result) {
        $q3 = "update session set numero = '$numeroAgendado' where idusuario = $idusuario";
        $result3 = mysqli_query($mysqli, $q3);
        if ($result3) {
            $q4 = "insert into historico values(null,'$numeroAgendado','tomado de agenda',$idusuario,'$fecha','$chk')";
            $result4 = mysqli_query($mysqli, $q4);
            if ($result4) {
                $qLocalidad = "select dep_localidad from numeros where numero = '$numeroAgendado'";
                ...
                if ($devuelve > 0) {
                    $response = array('result' => 'NumeroLlamarAgendado', 'numero' => $numeroAgendado, 'message' => 'Correcto.', 'localidad' => $localidad);
                } else {
                    $response = array('result' => 'NumeroLlamarAgendado', 'numero' => $numeroAgendado, 'message' => 'Correcto.', 'localidad' => 'Localidad Desconocida');
                }
            }
        }
    }
