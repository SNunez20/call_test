    ...
    $resultBorrarReferido = mysqli_query($mysqli,$qBorrarReferido);
        ...
        if($result3){
            ...
            if($result4){
                    ...
                    $qGrupo = "select * from relacion where idgrupousuarios=$grupoUsuario";
                    $resultGrupo = mysqli_query($mysqli,$qGrupo);
                    if($resultGrupo){
                    $fila = 0;
                    while ($reg = mysqli_fetch_array($resultGrupo)) {
                        $grupo[] = $reg['idgrupotel'];
                        $fila++;
                    }
                        if(empty($grupo)){
                            $primergrupo = 'sin grupo';
                        }else{
                            $primergrupo = $grupo[0];  
                        }
                    }
                    ...
                    $q6 = "select * from numeros where (grupo='$primergrupo'";
                    for($i=1;$i<count($_SESSION['grupotel']);$i++){
                        $valor = $_SESSION['grupotel'][$i];
                        $q6.=" or grupo = '$valor'";
                    }
                    $q6.=") and (flag='libre') order by rand() LIMIT 1";
                    $result6 = mysqli_query($mysqli,$q6);
                    $devuelve = mysqli_num_rows($result6);
                    if($devuelve>0){
                        while ($row = mysqli_fetch_assoc($result6)) {
                            $numero = $row['numero'];
                            $localidad = $row['dep_localidad'];
