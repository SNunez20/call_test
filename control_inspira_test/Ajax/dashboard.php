...
while ($row = mysqli_fetch_assoc($result)) {
    $id = $row["id"];
    $query = "SELECT fecha FROM historico_venta WHERE id_estado IN (1, 672, 684, 687, 691) AND id_cliente = $id ORDER BY fecha DESC LIMIT 1"; //compe
    $fecha_afiliacion = "";
    ...
    $params = "";
    $cedula = $row["cedula"];
    $query = "SELECT observaciones FROM padron_producto_socio WHERE cedula = '$cedula' GROUP BY cedula ORDER BY accion ASC";
...
function getIdConvenio($idSocio)
{
    ...
    $qSelect = "SELECT id_convenio_especial FROM relacion_socio_convenio_especial WHERE id_socio = $idSocio";
...
function getConveniosEspeciales()
{
    ...
    $qSelect = "SELECT * FROM convenios_especiales";
    $query = $mysqli->query($qSelect);
