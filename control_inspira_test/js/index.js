...
const sistema_rutas = true;

$("document").ready(function () {
  verifySession();

  listarPatologias("patologiaCobro");

  const verificarDatosSocioUrl = getUrl("verificar_datos_socio");

  if (verificarDatosSocioUrl != false) {
    verDatosSocio(Number.parseInt(verificarDatosSocioUrl), false);
  }

  const verFormularioCobroUrl = getUrl("ver_formulario_cobro");

  if (verFormularioCobroUrl != false) {
    verFormularioCobro(Number.parseInt(verFormularioCobroUrl));
  }
  ...
  if (sistema_rutas) $("#btnConfirmarCobro").click(verificarDireccionLatLng);
  else $("#btnConfirmarCobro").click(confirmarCobro);
  ...
  $(document.body).on("change", ".hservicio", function () {
    let id_listado_actual = localStorage.getItem("id_listado_actual");
    ...
    if ($mail_tit !== null && $ced_tit !== null && $cuotas !== null) {
      $.ajax({
        ...
        success: function (content) {
          swal(
            content.titulo_mensaje,
            content.mensaje,
            content.tipo_mensaje
...
function dashboard(id_listado_actual) {
  localStorage.setItem("id_listado_actual", id_listado_actual);

  let nombre_listado_actual =
    id_listado_actual == 1 ? "DASHBOARD VIDA" : "DASHBOARD INSPIRA";
  $("#span_identificar_listado_actual").text(nombre_listado_actual);
  let btn_cambiar_listado =
    id_listado_actual == 1
      ? "<button class='btn btn-primary' style='font-family: Verdana;' onclick='dashboard(2)'>Inspira</button>"
      : "<button class='btn btn-primary' style='font-family: Verdana;' onclick='dashboard(1)'>Vida</button>";
  $(`#div_btn_cambiar_listado`).html(`${btn_cambiar_listado}`);

  $(`#select-filter-1`).html(`<option value="">RUTA</option>`);
  $(`#select-filter-2`).html(`<option value="">SUCURSAL</option>`);
  $(`#select-filter-3`).html(`<option value="">MÉTODO DE PAGO</option>`);
  $(`#select-filter-5`).html(`<option value="">TIPO DE AFILIACIÓN</option>`);
  ...
  let nombre_archivo =
    id_listado_actual == 1 ? "dashboard" : "inspira/dashboard_inspira";

  $table = $("#table-dashboard").DataTable({
    ajax: {
      url: `Ajax/${nombre_archivo}.php`,
      data: {
...
function history(id_listado_actual) {
  localStorage.setItem("id_listado_actual", id_listado_actual);

  let nombre_listado_actual =
    id_listado_actual == 1 ? "HISTÓRICO VIDA" : "HISTÓRICO INSPIRA";
  $("#span_identificar_historial_actual").text(nombre_listado_actual);
  let btn_cambiar_listado =
    id_listado_actual == 1
      ? "<button class='btn btn-primary' style='font-family: Verdana;' onclick='history(2)'>Inspira</button>"
      : "<button class='btn btn-primary' style='font-family: Verdana;' onclick='history(1)'>Vida</button>";
  $(`#div_btn_cambiar_historial`).html(`${btn_cambiar_listado}`);

  let nombre_archivo =
    localStorage.getItem("id_listado_actual") == 1
      ? "history"
      : "inspira/history_inspira";
...
function seleccionarRuta(id) {
  ...
  let nombre_archivo =
    id_listado_actual == 1
      ? "obetenerDatosSocio"
      : "inspira/obetenerDatosSocio_inspira";
...
function calcularPrecio(
  ...
  let id_producto = $("#id_padron_producto").val();

  $data = `id_producto=${id_producto}&servicio=${servicio}&hrs_sanatorio=${hrs_sanatorio}&hrs_servicio=${hrs_servicio}&localidad=${localidad}&fecha_nacimiento=${fecha_nacimiento}&base=${base}`;
  ...
  let nombre_archivo =
    id_listado_actual == 1
      ? "calcularPrecio"
      : "inspira/calcularPrecio_inspira";
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      let promoMadre = response.socio.promoMadre
        ...
        : "";
      let promoVISA = response.socio.promoVISA
        ? `<br><span class="badge d-block badge-primary p-2 mx-auto mb-2" style="font-size: 20px">Promo VISA</span>`
        : "";
      let promoFloreada = response.socio.promoFloreada
        ? `<br><span class="badge d-block badge-primary p-2 mx-auto mb-2" style="font-size: 20px">Promo Floreada</span>`
        : "";
      $("#primaria").hide();
      if (response.result) {
        ...
        const fields = [
          ...
          ["cedula_titular_gf", "Titular del grupo familiar"],
        ];
...
function verFormularioCobro(id) {
  ...
  let nombre_archivo =
    id_listado_actual == 1
      ? "obtenerDatosPago"
      : "inspira/obtenerDatosPago_inspira";
    ...
    success: function (response) {
      ...
      if (response.result) {
        ...
        let badgePromoMadre = datos.promoMadre
          ...
          : "";
        let badgePromoVISA = datos.promoVISA
          ? `<span class="badge badge-primary d-block p-2 mx-auto mb-2" style="font-size: 20px">Promo VISA</span>`
          : "";
        ...
        let badgePagoAdelantado =
          datos.origenVenta == "6"
            ...
            : "";
        obtenerPatologiaSocio(datos.cedula);
        $("#observacionCobro").val("");
...
function verificarDireccionLatLng(e = false) {
  if (e !== false) e.preventDefault();

  $.ajax({
    type: "POST",
    url: "sistema_rutas/ajax/app/verificar_socio_con_latlng.php",
    data: { id_socio: $("#id_socio").val() },
    async: false,
    success: function (respuesta) {
      if (respuesta.error) {
        ...
        txt.innerHTML =
          respuesta.mensaje ||
          "No se pudo verificar si el socio cuenta con la direccion en Maps";
        swal({
          title: "Error en Confirmar Dirección",
          icon: "warning",
          html: true,
          content: txt,
          button: "Cerrar",
...
// Funcion que realiza el cobro
function confirmarCobro(e = false) {
  if (e !== false) e.preventDefault();
  ...
  const documentoSocio = $("#cedula_pago").val();
  const patologia = $("#patologiaCobro").val();
  const observacionPatologia = $("#observacionPatologiaCobro").val();
  ...
  } else if (observacionPatologia == "") {
    $(".error-cobro")
      .text("")
      .append("Se debe agregar una observación de la patología");
    $("#error-cobro").show().fadeOut(15000);
  } else {
    asociarPatologiaSocio(documentoSocio, patologia, observacionPatologia);
          ...
          if (localStorage.estado != 8) {
            ...
            buttons.morosidad = {
              ...
              action: function () {
                ...
                continuarProceso(id, 2);
                $("#modal-formulario-cobro").modal("hide");
                let id_listado_actual =
                  localStorage.getItem("id_listado_actual");
                dashboard(id_listado_actual);
              },
            };
...
function cobroAprobado(id, ob, isMercadoPago, origen) {
  ...
  let nombre_archivo =
    id_listado_actual == 1
      ? "procesoCobroAprobado"
      : "inspira/procesoCobroAprobado_inspira";
...
function guardar(id) {
  let retorno = null;
  retorno = localStorage.metodo_pago == "2" ? validarTarjeta() : true;
  ...
  if (retorno) {
    ...
    let nombre_archivo =
      id_listado_actual == 1
        ? "procesoActualizarSocio"
        : "inspira/procesoActualizarSocio_inspira";
...
function verProductosSocio(id) {
  ...
  let nombre_archivo =
    id_listado_actual == 1
      ? "obtenerProductosSocio"
      : "inspira/obtenerProductosSocio_inspira";
  ...
  $.ajax({
    ...
    success: function (response) {
      if (response.result) {
        ...
        localStorage.setItem(
          "promoCompetenciaVeintitres",
          response.dtoCompetenciaVeintitres
        ); //compe
        localStorage.setItem(
          "totalDtoCompetenciaVeintitres",
          response.totalDtoCompetenciaVeintitres
        ); //compe
...
function listarBeneficiarios(nroServicio) {
  ...
  let nombre_archivo =
    id_listado_actual == 1
      ...
      : "inspira/obtenerBeneficiarios_inspira";
  ...
  if (valEdades[0]) {
    ...
    let nombre_archivo =
      id_listado_actual == 1
        ...
        : "inspira/actualizarBeneficiarios_inspira";
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.result) {
          ...
          $("#abm_cedula_titular_gf").val(datos.cedula_titular_gf);
...
function listarServiciosSocio(productosSocio, productosResumen) {
  ...
  productosResumen.forEach(function (producto) {
    if (producto.accion == "1") {
      ...
      if (producto.nro_servicio == "01") {
        if (id_listado_actual == 1) {
          switch (producto.promo) {
            case "20":
              promoNombre = "promo: NP17";
              break;
            case "31":
              promoNombre = "promo: Competencia 2023";
              break;

            case "35":
              promoNombre = "promo: Competencia";
              break;

            case "2031":
              promoNombre = "promo: NP17 + Competencia 2023";
              break;

            case "2035":
              promoNombre = "promo: NP17 + Competencia";
              break;
          }
        } else {
          switch (producto.promo) {
            ...
            case "22":
              promoNombre = "promo: PCI";
              break;

            case "73":
              promoNombre = "promo: PCI - Bienestar y Asistir";
              break;
          }
        }

        if (!existepromo)
          switch (producto.dato_extra) {
            case "1":
              datoExtra = "(Competencia)";
              break;
            case "2":
              datoExtra = "(Herencia)";
              break;
            case "4":
              ...
              break;
            case "5":
              datoExtra = "(Competencia 2023)";
              break;
          }
      }

      let precioProducto = producto.importe;

      if ([35, 2035, 3335].includes(parseInt(producto.promo)))
        precioProducto += ` - con dcto al 10% del total -> <strong class="text-bold">UY$${Math.ceil(
          producto.importe * 0.1
        )}</strong>`;
      else if ([31, 2031].includes(parseInt(producto.promo)))
        precioProducto += ` - con dcto al 50% del total -> <strong class="text-bold">UY$${Math.ceil(
          producto.importe * 0.5
        )}</strong>`;

      productosNuevos += `
        <div>
        <p class="font-weight-bolder">${datoExtra}</p>
          <p>${producto.nombre_servicio}
          ${promoNombre}  
          (${producto.horas}hs) - UY$ ${precioProducto} 
        ${btnBeneficiarios}
  ...
  for (let i = 0; i < productosSocio.length; i++) {
    ...
    if (!productos_acotados.includes(+productosSocio[i].nro_servicio)) {
      ...
      if (id_listado_actual == 2) {
        if (productosSocio[i].cod_promo == "20") {
          //conva
          opcpromo += `<option value="0">Seleccione</option>
                    <option value="20" selected>NP17</option>
                    <option value="22">PCI</option>
                    <option value="73">PCI - Bienestar y Asistir</option>`;
        } else if (productosSocio[i].cod_promo == "22") {
          opcpromo += `<option value="0">Seleccione</option>
                    <option value="20">NP17</option>
                    <option value="22" selected>PCI</option>
                    <option value="73">PCI - Bienestar y Asistir</option>`;
        } else if (productosSocio[i].cod_promo == "73") {
          opcpromo += `<option value="0">Seleccione</option>
                    <option value="20">NP17</option>
                    <option value="22">PCI</option>
                    <option value="73" selected>PCI - Bienestar y Asistir</option>`;
        } else {
          opcpromo += `
          <option value="0" selected>Seleccione</option>
          <option value="20">NP17</option>
  ...
  if (["bienvenida", "morosidad"].includes(tipoAdmin)) {
    $("#divComentario").hide();
  ...
  let totalMostrar = localStorage.total_general;

  if (localStorage.promoCompetencia == "true")
    totalMostrar = `${localStorage.total_general} - Dcto Competencia -> UY$${localStorage.totalDtoCompetencia}`;
  else if (localStorage.promoCompetenciaVeintitres == "true")
    totalMostrar = `${localStorage.total_general} - Dcto Competencia 2023 -> UY$${localStorage.totalDtoCompetenciaVeintitres}`;
    ...
    swal({
      ...
      if (ok) {
        ...
        let nombre_archivo =
          id_listado_actual == 1
            ? "EliminarServicio"
            : "inspira/EliminarServicio_inspira";
...
let array_importes_actualizar_productos_inspira = [];
function ActualizarProductos() {
  ...
  array_importes_actualizar_productos_inspira = [];
  $("#datos_productos .importe").each(function () {
    valor = $(this).val();
    array_importes_actualizar_productos_inspira.push(valor);
  });
  ...
  $(".productos_socio").each(function (index, element) {
    ...
    if (id_listado_actual == 1) {
      ...
      arrProductos[c] = {
        ...
        array_importes: array_importes_actualizar_productos_inspira,
        cod_promo: promo,
        observacion: observacion,
        accion: accion,
        limit: limit,
    ...
    if (idProducto == "1" && promo != codpromo) cambiopromo = true;
  });
  ...
  if (!hayCambio && !cambiopromo) {
    ...
    let nombre_archivo =
      id_listado_actual == 1
        ? "actualizarServicios"
        : "inspira/actualizarServicios_inspira";

    if (id_listado_actual == 2) {
      let total_suma = 0;
      array_importes_actualizar_productos_inspira.map((val) => {
        total_suma = total_suma + val;
      });
      $("#total").text(total_suma);
    }
...
function actualizarDatosSocio() {
  ...
  let cedula_titular_gf = $("#abm_cedula_titular_gf").val();
...
function listarPatologias(idSelect) {
  $.ajax({
    url: "./Ajax/listarPatologias.php",
    dataType: "JSON",
    success: function (patologias) {
      const select = $(`#${idSelect}`);
      select.empty();
      patologias.map((_patologia) => {
        const { id_patologia, patologia } = _patologia;
        select.append(
          $("<option>", {
            value: id_patologia,
            text: patologia,
        ...
        $("#patologiaCobro").val(1);
      });
    },
  });
}

function asociarPatologiaSocio(documentoSocio, idPatologia, observacion) {
  $.ajax({
    type: "POST",
    url: "./Ajax/insertarPatologia.php",
    data: { documentoSocio, idPatologia, observacion },
    dataType: "JSON",
    success: function (r) {
      $("#patologiaCobro").val(1);
      $("#observacionPatologiaCobro").val("");
    },
  });
}

function obtenerPatologiaSocio(documentoSocio) {
  $.ajax({
    type: "POST",
    url: "./Ajax/obtenerPatologiaSocio.php",
    data: { documentoSocio },
    dataType: "JSON",
    success: function (r) {
      $("#patologiaCobro").val(r.idPatologia);
      $("#observacionPatologiaCobro").val(r.observacion);
    },
  });
}
