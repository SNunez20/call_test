...
$qTotalAltas = "SELECT  IFNULL(count(p.id),0) AS cantidad_altas, IFNULL(sum(p.total_importe),0) AS monto_ventas
...
WHERE p.estado <> 6 AND cast(pp.fecha_registro as date) >= '$fecha_desde' AND cast(pp.fecha_registro as date) <= '$fecha_hasta' AND p.alta ='0'  AND pp.abm='ALTA-PRODUCTO'
GROUP BY pp.cedula";
...
if ($rIncrementosFilial->num_rows !== 0) {
    ...
    while ($row = mysqli_fetch_array($rIncrementosFilial)) {
        ...
        INNER JOIN filiales f ON p.sucursal = f.nro_filial
        INNER JOIN padron_producto_socio pp ON p.cedula = pp.cedula
        where p.estado <> 6 AND cast(pp.fecha_registro as date) >= '$fecha_desde' AND cast(pp.fecha_registro as date) <= '$fecha_hasta' AND p.alta ='0'  AND pp.abm='ALTA-PRODUCTO' AND p.sucursal=$suc
        GROUP BY p.cedula";
...
if ($rIncrementosCall->num_rows !== 0) {
    while ($row = mysqli_fetch_array($rIncrementosCall)) {
        ...
        INNER JOIN filiales f on p.sucursal = f.nro_filial
        INNER JOIN padron_producto_socio pp on p.cedula = pp.cedula
        where p.estado <> 6 AND cast(pp.fecha_registro as date) >= '$fecha_desde' AND cast(pp.fecha_registro as date) <= '$fecha_hasta' AND p.alta ='0'  AND pp.abm='ALTA-PRODUCTO' AND g.id=$idcall
