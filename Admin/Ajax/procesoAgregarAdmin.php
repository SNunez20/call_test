...
if(isset($_SESSION['idadmin'])){
    ...
    $nombre =str_replace("'","",$data['nomAdmin']);
    $cedula =str_replace("'","",$data['cedAdmin']);
    if(isset($data['grupoAdmin'])){
        $grupo = str_replace("'","",$data['grupoAdmin']);
    }else{
        $grupo = $grupoAdmin;
    }
    $q = "select * from admin where usuario = '$cedula'";
    $result = mysqli_query($mysqli,$q);
    $devuelve = mysqli_num_rows($result);
    if($devuelve==0){
        $q2 = "insert into admin values(null,'$nombre','$cedula','$cedula',$grupo,'normal',null,1)";
