...
$q = "SELECT
        ...
        FROM
            ...
            INNER JOIN ( SELECT id, id_cliente, MAX( fecha ) AS fecha, id_estado FROM historico_venta WHERE id_estado = 672 GROUP BY id_cliente, id_estado ORDER BY id DESC ) AS h ON p.id = h.id_cliente 
            INNER JOIN usuarios AS u on p.id_usuario = u.id
            INNER JOIN gruposusuarios AS g ON u.idgrupo = g.id
        WHERE
            p.estado = 672 
            AND DATE_ADD( h.fecha, INTERVAL 120 HOUR ) < NOW()";
        ...
        $sBody = <<<HTML
            <html xmlns="https://www.w3.org/1999/xhtml">
                <head>
                    ...
                    <title>Promo competencia sin comprobante</title>
                </head>
                <body>
                    <h1>Promo competencia sin comprobante</h2>
                    <h2>Pasaron 5 días y aun no se sube el comprobante para el socio:</h2>
...
HTML;
        ...
        $mail->FromName = "Promo competencia";
        $mail->AddAddress("nicolas.g@vida.com.uy");
        $mail->AddCC("comercial@vida.com.uy");
        $mail->Subject  = "Promo competencia sin comprobante";
        $mail->Body     = $sBody;
        $mail->WordWrap = 50;
        $mail->Send();
...
$q = "SELECT
        ...
        WHERE
            p.estado = 672 AND
            DATE_ADD(h.fecha, INTERVAL 240 HOUR) < NOW()";
...
while($row = mysqli_fetch_array($result)){
    ...
    $q2 = "INSERT INTO 
            ...
            VALUES 
                ($id_usuario_sistema, $id_afiliado, 676, NOW(), 'PROMO COMPETENCIA VENCIDA POR EL SISTEMA', 11)";
