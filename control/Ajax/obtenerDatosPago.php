...
if (isset($_POST["typeAdmin"])) {
  ...
  $query = "SELECT
    pd.nombre, pd.cedula, pd.sucursal, pd.radio, pd.tipo_tarjeta, pd.numero_tarjeta,pd.email_titular,
    pd.telefono_titular, pd.nombre_titular, pd.cedula_titular, pd.anio_e, pd.mes_e, pd.cuotas_mercadopago,
    pd.total_importe, pd.tarjeta_vida, pd.metodo_pago, pd.cvv, m.metodo, pd.alta, pd.estado, pd.id_usuario,
    u.idgrupo, pd.fechafil, pd.localidad
  ...
  if ($result = mysqli_query($mysqli, $query)) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      $qOmt = "SELECT
            ...
            WHERE servicio='70' AND cedula_titular_gf = '$cedula'";
      ...
      $qPromoMadre = "SELECT
            cedula_titular_gf
            FROM padron_producto_socio
            WHERE cod_promo = 27 AND cedula = '$cedula'";
      ...
      $qPromoVISA = "SELECT
            `id`
            FROM `padron_producto_socio`
            WHERE `cod_promo` = 28 AND `cedula` = '$cedula'";
      $rPromoVISA = mysqli_query($mysqli, $qPromoVISA);
      $promoVISA = mysqli_num_rows($rPromoVISA) > 0;

      if ($alta == '1') { //compe

        $qPromoCompetencia = "SELECT
          importe,cod_promo
          FROM padron_producto_socio
          WHERE abm='ALTA' AND cedula = '$cedula'";
        ...
        $qRechazo = "SELECT
                ...
                WHERE id_cliente = $id AND fecha >= '$fechafil' AND fecha <='$fechaHoy' AND id_estado = 675";
