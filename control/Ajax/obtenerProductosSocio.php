...
if (isset($_POST["typeAdmin"])) {
    ...
    $esPromoCompetenciaVeintitres = false; //compe2023
    $totalConDtoCompetenciaVeintitres = 0; //compe2023
    $dtoCompetenciaVeintitres = 0.50; //compe

    // QUERY PARA EL RESUMEN DE PRODUCTO ,
    $query = "SELECT
    pd.id, pd.servicio, s.nombre_servicio, s.id as id_servicio, sum(hora) as horas, sum(importe) as total_importe,
    pd.accion as accion, ps.total_importe as total, pd.cod_promo as promo, ps.dato_extra, ps.alta
    ...
    $productosResumen = [];
    $codPromoCompetenciaVeintitres = ['31', '2031'];
    if ($result = mysqli_query($mysqli, $query)) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $esPromoCompetencia = (!$esPromoCompetencia && in_array($promo, ['35', '2035', '3335']))
                ? true : $esPromoCompetencia; //compe
            $totalConDtoCompetencia += ($esPromoCompetencia && $nroServicio == '01')
                ? round($totalImporte * $dtoCompetencia) : $totalImporte; //compe


            $esPromoCompetenciaVeintitres =
                $esPromoCompetenciaVeintitres || in_array($promo, $codPromoCompetenciaVeintitres);

            $totalConDtoCompetenciaVeintitres += ($esPromoCompetenciaVeintitres
                && in_array(
                    $promo,
                    $codPromoCompetenciaVeintitres
                ))
                ? round((int)$totalImporte * $dtoCompetenciaVeintitres) : $totalImporte; //compe2023
    ...
    if ($result = mysqli_query($mysqli, $query)) {
        ...
        $response['totalDtoCompetenciaVeintitres'] = $totalConDtoCompetenciaVeintitres; //compe
        $response['dtoCompetenciaVeintitres']   = $esPromoCompetenciaVeintitres; //compe
