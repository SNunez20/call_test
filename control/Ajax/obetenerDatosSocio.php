...
if (isset($_POST["typeAdmin"])) {
    ...
    $query = "SELECT  ps.nombre, ps.cedula, ps.tel, ps.radio, ps.localidad, ps.sucursal, ps.direccion, ps.observaciones, ps.fecha_nacimiento, ps.numero_tarjeta, ps.tarjeta, ps.tipo_tarjeta, ps.nombre_titular, ps.cedula_titular, ps.telefono_titular, ps.anio_e, ps.mes_e, ps.cvv, ps.cuotas_mercadopago, ps.count, ps.email, ps.email_titular, ps.total_importe, ps.metodo_pago, ps.banco_emisor, b.banco, u.idgrupo, ps.fechafil, ps.id as id_socio, pp.numero_vendedor, pp.nombre_vendedor, ps.count, ps.ruta, ps.tarjeta_vida, ps.id_usuario, ps.alta, ps.estado as id_estado, e.estado, ps.origen_venta
      FROM bancos_emisores b, padron_datos_socio ps INNER JOIN estados e ON ps.estado = e.id
      INNER JOIN metodos_pago mp ON ps.metodo_pago = mp.id
      INNER JOIN filiales f ON ps.sucursal = f.nro_filial
      INNER JOIN padron_producto_socio pp ON ps.cedula = pp.cedula
