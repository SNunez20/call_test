...
function redirigirDatosPersonales(event) {
    if (event !== false) event.preventDefault();
    const url = getUrl('socio');
    const url_redirecct = getUrl('ver_formulario_cobro') ? 'ver_formulario_cobro' : 'verificar_datos_socio';
...
async function modificarDireccion(event) {
    ...
    const reglasAValidar = [
        ...
        { input: 'gps_rutas_solar', nombre: 'Solar', required: false, min: 2, max: 4 },
        { input: 'gps_rutas_manzana', nombre: 'Manzana', required: false, min: 0, max: 4 },
        { input: 'gps_rutas_referencia', nombre: 'Referencia', required: true, min: 2, max: 70 },
        { input: 'gps_rutas_apartamento', nombre: 'Apartamento', required: false, min: 2, max: 100 },
    ];
    ...
    const response = await http('modificar_direccion_ruta.php', 'POST', form);
...
async function buscar(e = false, cargando=false) {
    return new Promise(async (resolve) => {
        ...
        <p>
            <strong>
                ${calle} ${puerta} ${solar} ${manzana} ${esquina_original} , ${localidad_original}    
            </strong>
        </p>
        `
        );
        ...
        if (calle !== '') {
            ...
            if (manzana !== '' && solar !== '') {
                query = `${calle}   ${esquina} ${localidad}`;

                busqueda = await busquedaEnMapa({ query: query, fields: ['name', 'geometry'] });
                if (busqueda === false) {
                    error('No pudimos encontrar la direccion');

                }
            }
            else {
                query = `${calle} ${puerta}  ${esquina} ${localidad}`;

                busqueda = await busquedaEnMapa({ query: query, fields: ['name', 'geometry'] });

                if (busqueda === false) {
                    query = `${calle}   ${puerta}  ${localidad}`;

                    busqueda = await busquedaEnMapa({ query: query, fields: ['name', 'geometry'] });

                    if (busqueda === false) {
                        query = `${calle}    ${localidad}`;
